...
$(document).ready(() => {
	$('.slider').each(function() {
		...
		$(this).find('.slide').on('mousedown touchstart', (e) => {
			stopSlider()

			//saving the coordinates of the cursor to use them as the origin of our drag movement
			previousCursorPosition = coordinatesFromEvent(e)

			//returning false prevents the transparent thumbnail image from appearing by canceling the browser's built-in DRAG EVENT
			return false
		})
		$(this).find('.slide').on('mousemove touchmove', (e) => {
			if(previousCursorPosition != null) {
				const currentCursorPosition = coordinatesFromEvent(e),
							delta = {
								//calculating how much our cursor has moved since the last cursor movement event has fired
								x: currentCursorPosition.x - previousCursorPosition.x,
								y: currentCursorPosition.y - previousCursorPosition.y
							}

				//moving slides; make a function for this?
				$(this).find('.slide').css('left', '+='+delta.x+'px')

				//updating the coordinates of the cursor
				previousCursorPosition = currentCursorPosition
			}
		})
		$(this).find('.slide').on('mouseup touchend', (e) => {
